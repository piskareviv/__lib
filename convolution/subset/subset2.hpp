...
#undef FUCK
    ...
    void
    convolve_subset(int lg, const u32* ptr1, const u32* ptr2, u32* ptr_out) const {
        assert(lg >= 3);
        const auto mt = this->mt;
        const auto mts = this->mts;
        ...
        for (int i = 0; i < (1 << lg); i++) {
            ...
            vec1[cnt][i] = mt.mul<true>(ptr1[i], mt.r2);
            vec2[cnt][i] = mt.mul<true>(ptr2[i], mt.r2);
        }
        ...
        for (int i = 0; i < (1 << lg); i += 8 * K) {
            ...
            for (int x = 0; x <= lg; x++) {
                ...
                for (int y = 0; x + y <= lg; y++) {
                    int ind = x + y;
                    u32x8 a = load_u32x8(vec1[x] + i), b = load_u32x8(vec2[y] + i);
        ...
        for (int i = 0; i < (1 << lg); i++) {
            int cnt = __builtin_popcount(i);
            ptr_out[i] = mt.mul<true>(vec1[cnt][i], 1);
        }
        for (int i = 0; i <= lg; i++) {
            _mm_free(vec1[i]);
        }
