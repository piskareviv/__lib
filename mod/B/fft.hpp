...
namespace FFT {
    ...
    __attribute__((optimize("O3"))) void expand_w(int k) {
        while (w.size() < k) {
            int r = w.size();
    ...
    __attribute__((optimize("O3"))) void expand_w_rb(int k) {
        if (w_rb_sz < (1 << k - 1)) {
            if (w_rb != nullptr) {
                free(w_rb);
            }
    ...
    void butterfly_u32_x2(u32 a, u32 b, u32 wj, u32 *ap, u32 *bp) {
        a = bt.shrink(a);
    ...
    void butterfly_u32x8_x2(u32x8 a, u32x8 b, u32x8 wj, u32 *ap, u32 *bp) {
        a = bts.shrink(a);
    ...
    __attribute__((optimize("O3"))) [[gnu::noinline]] void fft_cum(int lg, u32 *data) {
        expand_cum(lg);
        const int n = 1 << lg;
    ...
    __attribute__((optimize("O3"))) void fft(int lg, u32 *data) {
        const int n = 1 << lg;
    ...
    __attribute__((optimize("O3"))) void ifft(int lg, u32 *data) {
        int n = 1 << lg;
        expand_w(lg);
        ...
        if (lg <= 4) {
            ...
            int k = 0;
            {
                ...
                const u64x4 wj1 = RC(u64x4, _mm256_setr_epi64x(w[1][0], w[1][1], w[1][0], w[1][1]));
                const u64x4 wj2 = RC(u64x4, _mm256_setr_epi64x(w[2][0], w[2][1], w[2][2], w[2][3]));
                const u32x8 wj3 = load_u32x8(w[3]);
